...
const uploadPayment = async (req, res) => {
  ...
  try {
    ...
    for (const old of existing) deleteFile(old.proof_image);
    ...
    for (const file of files) {
      await db.query(
        ...
        [order_id, file.filename, 'Menunggu']
      );
    }
...
/* ─────────────────────────────────────────
   ADMIN: Konfirmasi pembayaran (valid)
───────────────────────────────────────── */
...
/* ─────────────────────────────────────────
   ADMIN: Tolak pembayaran → ajukan refund
   rekening diambil dari profil customer,
   admin tidak perlu input manual lagi
───────────────────────────────────────── */
    ...
    if (refund_rekening) {
      await db.query(
        ...
        [
          user_id,
          req.params.order_id,
          'Pembayaran ditolak — refund sedang diproses',
          `Pembayaran pesanan #${String(req.params.order_id).padStart(4, '0')} ditolak (${reject_reason}). Refund ke rekening ${refund_bank} ${refund_rekening} a.n. ${refund_atas_nama} sedang diproses oleh tim kami.`,
          'Menunggu',
        ]
      );
    } else {
      await db.query(
        ...
        [
          user_id,
          req.params.order_id,
          'Pembayaran ditolak — mohon lengkapi rekening bank',
          `Bukti pembayaran pesanan #${String(req.params.order_id).padStart(4, '0')} ditolak (${reject_reason}). Silakan lengkapi nomor rekening bank kamu di halaman profil agar refund dapat diproses.`,
          'Menunggu',
        ]
      );
    }
...
/* ─────────────────────────────────────────
   CUSTOMER: Isi / update nomor rekening
───────────────────────────────────────── */
  ...
  try {
    ...
    await db.query(
      ...
      [
        ...
        `Nomor rekening kamu untuk refund pesanan #${String(order_id).padStart(4, '0')} sudah kami terima. Tim kami akan segera memproses refund.`,
        'Menunggu',
      ]
    );
...
/* ─────────────────────────────────────────
   SUPER ADMIN: Konfirmasi refund
───────────────────────────────────────── */
const confirmRefund = async (req, res) => {
  const order_id = req.params.order_id;
  const refund_proof = req.file?.filename || null;
  ...
  try {
    ...
    if (refund_proof) {
      deleteFile(payments[0].refund_proof);
      ...
      await db.query(
        ...
        [
          ...
          `Dana refund pesanan #${String(order_id).padStart(4, '0')} telah ditransfer ke rekening ${refund_bank} ${refund_rekening} a.n. ${refund_atas_nama}. Terima kasih telah berbelanja.`,
          'Selesai',
        ]
      );
...
/* ─────────────────────────────────────────
   ...
   PATCH: auto-upgrade Menunggu Rekening →
   Menunggu Konfirmasi Super Admin kalau
   customer sudah isi rekening di profil
───────────────────────────────────────── */
const getAllPayments = async (req, res) => {
  try {
    ...
    // Auto-upgrade: Menunggu Rekening → Menunggu Konfirmasi Super Admin
    // kalau user sudah isi rekening di profil tapi payments belum ter-update
    ...
    if (needsUpgrade.length > 0) {
      ...
      // Langsung update data yang dikembalikan tanpa perlu query ulang
      payments.forEach(p => {
...
/* ─────────────────────────────────────────
   GET: Detail pembayaran satu pesanan
───────────────────────────────────────── */
